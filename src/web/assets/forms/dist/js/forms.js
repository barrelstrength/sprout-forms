  ...
  getRowHtml: function getRowHtml(rowId, columns, baseName, values, conditionalTypes) {
    ...
    for (var colId in columns) {
      ...
      switch (col.type) {
        case 'select':
          {
            rowHtml += '<div class="select"><select name="' + _name + '">';
        ...
        case 'selectCondition':
          {
            conditionFieldName = _name;
            var colVal = typeof col.options[0] !== 'undefined' ? col.options[0].value : '';
            conditionFieldValue = value != '' ? value : colVal;
            rowHtml += '<div class="select"><select data-check-value-html="true" name="' + _name + '">';
            col.options = conditionalTypes[formFieldValue]['rulesAsOptions'];
            var _hasOptgroups = false;
            var _firstRow = 'selected';

            for (var _key in col.options) {
              var _option = col.options[_key];
              ...
              if (typeof _option.optgroup !== 'undefined') {
                ...
                rowHtml += '<optgroup label="' + _option.optgroup + '">';
              } else {
                var _optionLabel = typeof _option.label !== 'undefined' ? _option.label : _option,
                    _optionValue = typeof _option.value !== 'undefined' ? _option.value : _key,
                    _optionDisabled = typeof _option.disabled !== 'undefined' ? _option.disabled : false;

                rowHtml += '<option ' + _firstRow + ' value="' + _optionValue + '"' + (_optionValue == value ? ' selected' : '') + (_optionDisabled ? ' disabled' : '') + '>' + _optionLabel + '</option>';
              }
        ...
        case 'checkbox':
          {
            rowHtml += '<input type="hidden" name="' + _name + '">' + '<input type="checkbox" name="' + _name + '" value="1"' + (value ? ' checked' : '') + '>';
            break;
          }

        default:
          {
            rowHtml += '<input class="text fullwidth" type="text" name="' + _name + '" value="' + value + '">';
          }
      }
...
(function ($) {
  ...
  Craft.SproutForms.FieldLayoutEditor = Garnish.Base.extend({
    ...
    editField: function editField(currentOption) {
      var option = currentOption.currentTarget;
...
(function ($) {
  Craft.SproutForms.FormSettings = Garnish.Base.extend({
    ...
    editConditional: function editConditional(currentOption) {
      var option = currentOption.currentTarget;
    ...
    editIntegration: function editIntegration(currentOption) {
      var option = currentOption.currentTarget;
